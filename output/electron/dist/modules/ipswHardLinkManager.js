...
class IPSWHardLinkManager {
    ...
    async createLinkForFile(file) {
        console.log("[IPSWHardLinkManager] createLinkForFile:start", {
            name: file.name,
            path: file.path,
        });
        const parsed = this.parseIPSW(file.name);
        if (!parsed) {
            console.log("[IPSWHardLinkManager] skip: parseIPSW failed", file.name);
            return;
        }
        console.log("[IPSWHardLinkManager] parsed", parsed);
        const sourceExists = await promises_1.default
            .access(file.path)
            .then(() => true)
            .catch(() => false);
        if (!sourceExists) {
            console.log("[IPSWHardLinkManager] skip: source file not found", file.path);
            return;
        }
        ...
        console.log("[IPSWHardLinkManager] matched device names", {
            fileName: file.name,
            id: parsed.id,
            build: parsed.build,
            deviceNames,
        ...
        if (!deviceNames.length) {
            console.log("[IPSWHardLinkManager] skip: no device names matched", file.name);
            return;
        }
        ...
        console.log("[IPSWHardLinkManager] record", {
            ...record,
            formattedLinkPath,
        });
        if (this.records.get(file.path)?.linkPath === formattedLinkPath) {
            console.log("[IPSWHardLinkManager] skip: record already tracked", formattedLinkPath);
            return;
        }
        await promises_1.default.mkdir(this.folderPath, { recursive: true });
        const linkExists = await promises_1.default
            .access(record.linkPath)
            .then(() => true)
            .catch(() => false);
        console.log("[IPSWHardLinkManager] link path check", {
            linkPath: record.linkPath,
            linkExists,
        });
        if (linkExists) {
            console.log("[IPSWHardLinkManager] skip: link already exists", record.linkPath);
        ...
        try {
            console.log("[IPSWHardLinkManager] creating hard link", {
                source: file.path,
                linkPath: record.linkPath,
            });
            await promises_1.default.link(file.path, record.linkPath);
            if (record.linkPath !== formattedLinkPath) {
                console.log("[IPSWHardLinkManager] renaming hard link", {
                    from: record.linkPath,
                    to: formattedLinkPath,
                });
                await promises_1.default.rename(record.linkPath, formattedLinkPath);
            }
            console.log("[IPSWHardLinkManager] hard link created", formattedLinkPath);
            this.records.set(file.path, {
                ...record,
                linkPath: formattedLinkPath,
                fileName: path_1.default.basename(formattedLinkPath),
    ...
    groupDeviceNames(deviceNames) {
        ...
        if (!repeatedModels.length)
            return grouped;
        const normalized = [...new Set(repeatedModels)].sort();
