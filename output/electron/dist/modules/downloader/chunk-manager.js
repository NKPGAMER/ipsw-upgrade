...
const DEFAULT_CHUNK_SIZE = 64 * 1024 * 1024; // 64 MB per chunk
const WRITE_HIGH_WATER = 16 * 1024 * 1024; // 16 MB — flush threshold (larger = fewer syscalls)
...
class ChunkManager {
    ...
    constructor(state, stateManager, opts = {}) {
        ...
        this.opts = {
            maxConnections: opts.maxConnections ?? (isHDD ? 12 : 24),
            initialConnections: opts.initialConnections ?? 8,
    ...
    buildQueue() {
        ...
        this.pendingQueue = this.state.chunks
            .map((chunk, index) => ({
            ...chunk,
            size: chunk.size ?? Math.max(0, chunk.end - chunk.start + 1),
            priority: chunk.priority ?? index,
        }))
            .filter((c) => !c.completed)
            .sort((a, b) => b.size - a.size || a.priority - b.priority || a.index - b.index);
    }
    // ─── Main entry ─────────────────────────────────────────────────────────────
    ...
    static async fetchMetadata(url) {
        ...
        const pool = new undici_1.Pool(parsed.origin, { connections: 1, pipelining: 1, keepAliveTimeout: 30_000, headersTimeout: 15_000 });
        try {
