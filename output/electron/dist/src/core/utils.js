...
class ConfirmService {
    ...
    closeAll() {
        return new Promise((resolve, reject) => {
            try {
                ...
                const wrap = this.root.querySelector(".confirm-wrap");
                if (wrap) {
                    const backdrop = wrap.querySelector(".confirm-backdrop");
    ...
    show(item) {
        const { title = "", confirmText = "Ok", cancelText = "Cancel", variant = "default" } = item.options;
    ...
    `;
        ...
        const backdrop = wrap.querySelector(".confirm-backdrop");
        const dialog = wrap.querySelector(".confirm-dialog");
        const btnCancel = wrap.querySelector(".confirm-btn-cancel");
        const btnConfirm = wrap.querySelector(".confirm-btn-confirm");
        dialog.focus();
        const close = (result) => {
            backdrop.classList.replace("backdrop-in", "backdrop-out");
            dialog.classList.replace("dialog-in", "dialog-out");
...
exports.default = {
    ...
    showSuccessMessage(message, timeout = 4000) {
        ...
        successDiv.textContent = typeof message === 'string' ? message : (0, i18next_1.t)(message.id);
    ...
    customConfirm: (message, options) => confirmService.confirm(message, options),
