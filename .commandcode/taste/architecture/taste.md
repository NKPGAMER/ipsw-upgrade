...
- Prefers a centralized AppShell layout skeleton over per-page `fixed` positioning with CSS custom property offsets. The skeleton consists of: a permanently fixed Titlebar (never hidden or overridden by pages), a Sidebar whose visibility is controlled by page layout declarations (not route matching), and a single ContentArea as the render target for all pages/routes. Pages use natural sizing (`size-full`, `flex-1`) within the ContentArea rather than calculating titlebar/sidebar offsets. Confidence: 0.90

- Pages declare their layout intent via a `usePageLayout(layout)` hook with stack-based push/pop semantics — setting the layout on mount and automatically restoring the previous layout on unmount. This replaces route-matching arrays for sidebar visibility. The two layout states are: `"default"` (sidebar visible, content flex-1) and `"fullContent"` (sidebar hidden, content 100% width). Confidence: 0.85

- When `AnimatePresence` wraps page content with a `motion.div`, the wrapper must use definite sizing (`className="size-full"`) rather than `style={{ minHeight: "100%" }}`. `minHeight` produces an indeterminate computed height that breaks the CSS overflow/scroll chain — child elements cannot properly compute their own heights and scrolling fails when the window shrinks. Confidence: 0.75
