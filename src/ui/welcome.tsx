import { useState, useEffect, useCallback, useRef, memo, type FC, type JSX } from "react";
                ...
                <div className="flex flex-col gap-2 px-4! py-3!">
                    {[
                        ...
                        <div key={i} className="flex items-start gap-2.5">
                            ...
                            <div className="flex-1 text-[12.5px] leading-[1.45] text-[#c0c0c0]">{text}</div>
                        </div>
                    ))}
                </div>
            </div>
...
const PageDownload: FC<DownloadPageProps> = ({
    saveDir, setSaveDir, onBrowseSaveDir, skipVerify, setSkipVerify, onNext, onBack,
}) => {
    const { t } = useTranslation();
    return (
        ...
        <div className="flex-1 overflow-y-auto px-9! py-7!">
            ...
            <Section icon={IconTrash} title={t("wizard.firmware.section.cleanup")}>
                ...
                <Row
                    ...
                    right={<Toggle on={autoRemoveInvalid} onChange={setAutoRemoveInvalid} />}
                />
            </Section>
...
const BtnBack: FC<{ onClick: () => void; label: string }> = memo(({ onClick, label }) => (
    <button
        onClick={onClick}
...
const BtnNext: FC<{ onClick: () => void; label: string }> = memo(({ onClick, label }) => (
    <button
        onClick={onClick}
...
export default function App(): JSX.Element {
    ...
    useEffect(() => {
        ...
        ]).then(([savedFolder, savedSkipVerify, savedLinkEnabled, savedLinkOutDir, savedCleanOld, savedCleanDupe, savedCleanInvalid]) => {
    ...
    }, [saveDir, skipVerify, parseFileName, linkOutDir, autoRemoveOld, autoRemoveDupe, autoRemoveInvalid]);
