...
const detail = new class {
  ...
  private async handleRelatedFiles(relatedFiles: IPSWFile[], latestBuildId: string) {
    const { oldFiles, dupeFiles } = relatedFiles.reduce((acc, f) => {
      ...
      return acc;
    }, { oldFiles: [] as IPSWFile[], dupeFiles: [] as IPSWFile[] });
...
const downloadManager = {
  createDownloadItem: (progress: DownloadProgress): HTMLDivElement => {
    ...
    item.addEventListener('click', (e) => {
      ...
      if (action === 'pause') downloadManager.pauseDownload(progress.downloadId);
      else if (action === 'resume') downloadManager.resumeDownload(progress.downloadId);
      else if (action === 'cancel') downloadManager.cancelDownload(progress.downloadId);
    });

    return item;
  },

  updateDownloadItem: (progress: DownloadProgress) => {
    let item = elements.downloads.list.querySelector(`[data-download-id="${progress.downloadId}"]`) as HTMLDivElement;
    ...
    if (['completed', 'error', 'cancelled'].includes(progress.status)) {
      setTimeout(() => {
        ...
        if (progress.status === 'completed' && state.currentFolder) {
          window.api.getFiles(state.currentFolder).then((files: IPSWFile[]) => {
            data.localFiles = files;
            ui.updateStats();
            if (state.currentProduct) ui.loadDevices(state.currentProduct as Product);
          });
        }
      }, 5000);
    }
...
const initEventListeners = () => {
  window.downloader.onDownloadProgress((progress: DownloadProgress) => {
