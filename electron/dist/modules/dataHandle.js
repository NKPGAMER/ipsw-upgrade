...
class DataHandle {
    ...
    // ── Helpers ────────────────────────────────────────────────────────────────
    async resolveLatestRelease(caller) {
        if (this.latestRelease)
            return this.latestRelease;
        try {
    ...
    async hasLocalData({ type, identifier }) {
        if (type === "devices")
            return (await (0, userData_1.read)(FILES.devices)) !== null;
        if (!identifier)
            return false;
        const product = this.getProductType(identifier);
        if (!product)
            return false;
        return (await (0, userData_1.read)(`products/${product}/${identifier}.json`)) !== null;
    }
}
