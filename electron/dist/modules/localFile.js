...
async function scanFolder(folder) {
    let entries;
    try {
        entries = await fs_1.promises.readdir(folder);
    }
    catch {
        return [];
    }
    const ipswEntries = entries.filter(f => f.endsWith(".ipsw"));
    const files = await Promise.all(ipswEntries.map(async (f) => {
        const filePath = (0, path_1.join)(folder, f);
        try {
            const { size } = await fs_1.promises.stat(filePath);
