...
type DeviceState = "NORMAL_READY" | "NORMAL_UNTRUSTED" | "RECOVERY" | "DFU";
...
type DeviceEvent =
  ...
  | "apple-device-trusted"
  | "apple-device-untrusted";

// ---------------------------------------------------------------------------
// Paths
// ---------------------------------------------------------------------------

function getIdevicePath(): string {
  const isDev = !app.isPackaged;
  return isDev
    ? path.join(__dirname, "..", "..", "..", "resources", "idevice")
    : path.join(process.resourcesPath, "idevice");
}

const idevicePath = getIdevicePath();
const EXE = process.platform === "win32" ? ".exe" : "";
const ideviceIdPath = path.join(idevicePath, `idevice_id${EXE}`);
const ideviceInfoPath = path.join(idevicePath, `ideviceinfo${EXE}`);
const irecoveryPath = path.join(idevicePath, `irecovery${EXE}`);
 ...
 */
async function getConnectedUDIDs(): Promise<string[]> {
  try {
...
async function isDeviceTrusted(udid: string): Promise<boolean> {
  try {
...
class DevicePoller {
  ...
  constructor(
    ...
    private readonly onUdidDiscovered: (udid: string) => void,
    private readonly onTrusted: (udid: string) => void,
    private readonly onTimeout: (udid: string | null) => void,
    private getCurrentUdid: () => string | null,
  ) {}
  ...
  private async poll() {
    ...
    try {
      ...
      if (!this.destroyed) this.scheduleNext(POLL_INTERVAL_MS);
    } finally {
      this.running = false;
    }
  }
   ...
   */
  private resolveUdid(udids: string[]): string | null {
  ...
  setUdidResolver(fn: (udids: string[]) => string | null) {
    this.resolveUdid = fn;
  }
}
...
class AppleDevice {
  ...
  private connectedDevices: Map<string, AppleDeviceInfo> = new Map();

  /** Secondary index: udid → bus key. For fast reverse-lookup. */
  private udidToKey: Map<string, string> = new Map();

  /** Active pollers, keyed by bus key. */
  private pollers: Map<string, DevicePoller> = new Map();
   ...
   */
  private attachQueue: usb.Device[] = [];
  ...
  constructor(window?: BrowserWindow) {
    ...
    usb.on("attach", this.onDeviceConnect.bind(this));
    usb.on("detach", this.onDeviceDisconnect.bind(this));

    // Kick off the initial scan asynchronously.
    // Errors are caught internally; the constructor stays synchronous.
    this.scanExistingDevices().catch((err) =>
      console.error("[scan] Initial scan failed:", err),
    );
  }
  ...
  private async scanExistingDevices(): Promise<void> {
    ...
    await Promise.all(
      knownUdids.map(async (udid) => {
        if (await isDeviceTrusted(udid)) trustedSet.add(udid);
      }),
    );

    // Walk USB bus — only Apple devices
    const usbDevices = usb.getDeviceList().filter(
      (d) => d.deviceDescriptor.idVendor === config.appleVendorId,
    );
    ...
    for (const device of usbDevices) {
      ...
      if (this.connectedDevices.has(key)) continue;
      ...
      const matchedUdid =
        [...unmatchedUdids].find((u) => trustedSet.has(u)) ??
        [...unmatchedUdids][0] ??
        null;
    ...
    for (const udid of unmatchedUdids) {
      const key = `wifi-${udid}`;
      if (this.udidToKey.has(udid)) continue;

      const state: DeviceState = trustedSet.has(udid) ? "NORMAL_READY" : "NORMAL_UNTRUSTED";
      console.log(`[scan] Found Wi-Fi device: ${udid} (${state})`);

      // usb.Device is not available for Wi-Fi; cast to satisfy the type
      this.connectedDevices.set(key, { udid, state, usbDevice: null as unknown as usb.Device });
  ...
  private getDeviceKey(device: usb.Device): string {
    return `${device.busNumber}-${device.deviceAddress}`;
  }

  private sendToRenderer(event: DeviceEvent, payload: unknown) {
    if (this.window && !this.window.isDestroyed()) {
      this.window.webContents.send(event, payload);
    }
  }

  private getUsbState(device: usb.Device): DeviceState | null {
    const pid = device.deviceDescriptor.idProduct;
    if (DFU_MODE_PIDS.includes(pid)) return "DFU";
    if (pid === RECOVERY_MODE_PID) return "RECOVERY";
    return null;
  }

  /** UDIDs already claimed by other devices in this session. */
  private claimedUdids(): Set<string> {
  ...
  private startTrustPolling(key: string) {
    if (this.pollers.has(key)) return; // already polling
  ...
  private async onDeviceConnect(device: usb.Device) {
    if (device.deviceDescriptor.idVendor !== config.appleVendorId) return;
    ...
    if (!this.connectedDevices.has(key)) return;

    this.startTrustPolling(key);
  }

  private onDeviceDisconnect(device: usb.Device) {
    if (device.deviceDescriptor.idVendor !== config.appleVendorId) return;
  ...
  public getDevices(): AppleDeviceInfo[] {
    return Array.from(this.connectedDevices.values());
  }

  public async runIdeviceCommand(
    bin: string,
    args: string[],
    udid?: string,
  ): Promise<string> {
    const binPath = path.join(idevicePath, `${bin}${EXE}`);
   ...
   */
  public async getDeviceState(udid: string): Promise<DeviceState> {
   ...
   */
  public async getDeviceInfo(udid: string, key: string): Promise<string> {
    // Resolve state from registry; fall back to a live USB check.
    const state: DeviceState = await this.getDeviceState(udid);
    ...
    switch (state) {
      ...
      case "NORMAL_UNTRUSTED":
        throw new Error(
          `Device ${udid} is connected but not yet trusted. ` +
   ...
   */
  private static readonly RECOVERY_ALLOWED_KEYS = new Set(["MODE", "PRODUCT", "NAME"]);

  /**
   * Run `irecovery -q` and extract a single allowed key from the output.
   * Output format per line: `KEY: value`
   */
  private async getRecoveryInfo(key: string): Promise<string> {
    const upperKey = key.toUpperCase();

    if (!AppleDevice.RECOVERY_ALLOWED_KEYS.has(upperKey)) {
      throw new Error(
        `Key "${key}" is not accessible in RECOVERY/DFU mode. ` +
    ...
    for (const line of output.split("\n")) {
      ...
      if (lineKey === upperKey) return lineValue;
    }
   ...
   */
  private async resolveStateLive(udid: string): Promise<DeviceState> {
    // 1. Check recovery / DFU via irecovery -m
    try {
      const mode = (await runCommand(irecoveryPath, ["-m"])).toLowerCase();
      if (mode.includes("dfu")) return "DFU";
    ...
    const udids = await getConnectedUDIDs();
    if (udids.includes(udid)) return "NORMAL_UNTRUSTED";
...
export { AppleDevice, DeviceState, AppleDeviceInfo };